...
import {
  Search, Heart, Star, Plus, Filter as FilterIcon, ChevronDown,
  SlidersHorizontal, ToggleRight, ToggleLeft, X, Scan
} from 'lucide-react';
...
const MOCK_IMAGES = [
  'https://images.unsplash.com/photo-1576671081837-49000212a370?auto=format&fit=crop&q=80&w=400',
...
function buildFdaQuery(term: string): string[] {
...
function scoreRelevance(brand: string, generic: string, searchTerms: string[]): number {
  let score = 0;
  ...
  for (const term of searchTerms) {
    ...
    for (const w of words) {
      if (w.length > 1 && (brandLower.includes(w) || genericLower.includes(w))) wordHits++;
    }
    if (words.length > 0) score += Math.round((wordHits / words.length) * 30);
  }
...
export const MedicineExplorer = () => {
  ...
  // Medicines sourced from prescription scan / manual entry
  ...
  useEffect(() => {
    fetchDrugs(searchQuery, prescriptionMedicines);
  ...
  /** Called by PrescriptionScanModal when user confirms medicines */
  const handlePrescriptionMedicinesConfirmed = (medicines: string[]) => {
    setPrescriptionMedicines(medicines);
    fetchDrugs(searchQuery, medicines);
  };

  const removePrescriptionMedicine = (med: string) => {
    setPrescriptionMedicines((prev) => {
      const updated = prev.filter((m) => m !== med);
      fetchDrugs(searchQuery, updated);
      return updated;
    });
  };
  ...
  // ── Scan button used in both header & desktop bar ──
  const ScanButton = ({ className = '' }: { className?: string }) => (
    <button
      id="explorer-scan-prescription-btn"
      type="button"
      onClick={() => setIsScanModalOpen(true)}
      className={`relative flex items-center gap-2 px-4 py-2.5 rounded-xl font-bold text-sm transition-all active:scale-95 ${
  ...
  // ── Prescription medicine chips strip ──
  const PrescriptionChips = () => {
    if (prescriptionMedicines.length === 0) return null;
    return (
      <div className="flex flex-wrap items-center gap-2 mt-2 p-3 bg-primary/5 border border-primary/15 rounded-2xl">
        <div className="flex items-center gap-1.5 text-[11px] font-black text-primary uppercase tracking-wider shrink-0">
          <Scan size={11} />
          <span>Rx</span>
        </div>
        {prescriptionMedicines.map((med) => (
          <span
            key={med}
            className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[12px] font-bold bg-white dark:bg-slate-800 text-slate-700 dark:text-slate-200 border border-slate-200 dark:border-slate-700 shadow-sm animate-in fade-in zoom-in duration-200"
          >
            {med}
            <button
              type="button"
              onClick={() => removePrescriptionMedicine(med)}
        ...
        <button
          ...
          className="ml-auto text-[11px] font-bold text-slate-400 hover:text-red-500 transition-colors flex items-center gap-1"
        >
          <X size={11} />
          Clear
        </button>
  ...
  return (
    <>
      ...
      <PrescriptionScanModal
        ...
        onMedicinesConfirmed={handlePrescriptionMedicinesConfirmed}
      />
        ...
        <div className="flex flex-col lg:flex-row gap-8 p-4 md:p-6 lg:p-8">
          ...
          <aside className={`fixed lg:sticky lg:top-[24px] lg:max-h-[calc(100vh-48px)] top-0 right-0 h-full lg:h-fit w-[85vw] max-w-[340px] lg:w-[300px] xl:w-[320px] shrink-0 bg-white dark:bg-slate-900 lg:rounded-3xl shadow-2xl lg:shadow-[0_8px_30px_rgb(0,0,0,0.03)] border-l lg:border border-slate-100 dark:border-white/5 p-6 z-50 transition-transform duration-300 overflow-y-auto ${isMobileFiltersOpen ? 'translate-x-0' : 'translate-x-full lg:translate-x-0'}`}>
            <div className="flex justify-between items-center mb-8 pb-5 border-b border-slate-100 dark:border-slate-800">
              ...
              <button
                onClick={() => setIsMobileFiltersOpen(false)}
                className="lg:hidden p-2 bg-slate-100 dark:bg-slate-800 rounded-full text-slate-500 hover:text-slate-700 dark:hover:text-white transition-colors"
              >
                <X size={20} />
              </button>
            </div>
                ...
                {CATEGORIES.map((cat) => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 rounded-xl text-sm transition-all duration-200 flex items-center gap-3 relative overflow-hidden group ${
                        isActive
                    ...
                    >
                      {!isActive && <span className="w-1.5 h-1.5 rounded-full bg-slate-300 dark:bg-slate-600 group-hover:bg-primary/50 transition-colors" />}
            ...
            <div className="mb-8">
              ...
              <div className="border border-slate-100 dark:border-slate-800 rounded-2xl p-5 mb-4 bg-slate-50 dark:bg-slate-800/50">
                ...
                <div className="relative h-1.5 bg-slate-200 dark:bg-slate-700 rounded-full mb-4">
                  ...
                  <div className="absolute left-0 top-1/2 -translate-y-1/2 w-4 h-4 bg-white border-[3px] border-primary rounded-full shadow-md cursor-pointer hover:scale-110 transition-transform" />
                  <div className="absolute right-0 top-1/2 -translate-y-1/2 w-4 h-4 bg-white border-[3px] border-primary rounded-full shadow-md cursor-pointer hover:scale-110 transition-transform" />
                </div>
          ...
          <div className="flex-1 w-full min-w-0">
            ...
            <div className="hidden lg:flex flex-col gap-3 mb-8">
              ...
              {/* Prescription chip strip — desktop */}
              <PrescriptionChips />
            </div>
            ...
            {loading ? (
              ...
              <div className="bg-white dark:bg-slate-900 rounded-3xl p-10 md:p-16 text-center border border-slate-100 dark:border-slate-800 shadow-sm flex flex-col items-center justify-center mx-auto max-w-lg mt-10">
                ...
                <p className="text-slate-500 font-medium max-w-sm leading-relaxed">
                  We couldn't find any products matching your current filters. Try adjusting your search, selecting a different category, or scanning a prescription.
                </p>
                <div className="flex gap-3 flex-wrap justify-center mt-6">
                  ...
                  <button
                    onClick={() => setIsScanModalOpen(true)}
                    className="flex items-center gap-2 px-6 py-2.5 bg-primary text-primary-foreground font-bold rounded-xl shadow-md shadow-primary/20 hover:bg-primary/90 transition-all active:scale-95"
                  >
                    <Scan size={16} />
                    Scan Prescription
                  </button>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4 gap-5 lg:gap-6">
                {drugs.map((drug, index) => {
                  ...
                  return (
                    <div
                      key={drug.id}
                      className="bg-white dark:bg-slate-900 rounded-3xl overflow-hidden border border-slate-100 dark:border-slate-800 shadow-[0_2px_10px_rgb(0,0,0,0.02)] hover:shadow-[0_12px_30px_rgb(0,0,0,0.06)] hover:border-slate-200 dark:hover:border-slate-700 transition-all duration-300 group flex flex-col relative h-full"
                    >
                      {/* Discount badge */}
                        ...
                        <div className="flex justify-between items-end mb-4 gap-2">
                          <div className="flex items-center gap-1.5 bg-amber-50 dark:bg-amber-500/10 px-2.5 py-1 rounded-md border border-amber-100 dark:border-amber-500/20">
                            ...
                            <span className="text-xs font-black text-amber-600 dark:text-amber-500">
                              ...
                              <span className="text-amber-600/50 dark:text-amber-500/50 font-semibold ml-0.5">
